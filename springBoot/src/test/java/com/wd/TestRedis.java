...
//        jedis.del("javaFramwork");
    ...
    public void setData() {
        for (int i = 0; i < 10; i++) {
            final String num = createUUIdUntil.createUUid();
            jedis.set(num, num);
        }
    }
    ...
    public void getdbSize() {
        System.out.println("获取数据库中key总数" + jedis.dbSize());
        Set set = jedis.keys("*");
        ...
        while (iterator.hasNext()) {
            ...
            saveRedisObject(jedis, keys, RedisPool.ADDR, RedisPool.PORT);   //实例化redis 中数据
     ...
     * 查询持久化策略
     */
    public List<String> getSaveConfig() {
     ...
     * 设置持久化策略
     */
    public String setSaveConfig() {
        final String celue_1 = "800  1";
        final String celue_2 = "400 2";
     ...
     * @param redisKey
     */
    public static void saveRedisObject(final Jedis jedis, final String redisKey, final String addr, final Integer port) {

        String redisType = jedis.type(redisKey);
        RedisTable redisTable = new RedisTable();
        redisTable.setRedisKey(redisKey);
        redisTable.setRedisType(redisType);
        Timestamp date  = new Timestamp(new Date().getTime());
        redisTable.setCreateTime(date);
        redisTable.setUpdateTime(date);
